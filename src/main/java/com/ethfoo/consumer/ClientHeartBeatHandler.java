...
public class ClientHeartBeatHandler extends
	...
	public ClientHeartBeatHandler(Bootstrap bootstrap,
			List<Channel> channelList, AddressProvider addressProvider) {
	...
	public void channelInactive(ChannelHandlerContext ctx) throws Exception {
		System.out.println("client channel inactive, this channel is: "
		...
		ctx.executor().scheduleAtFixedRate(new Runnable() {
			public void run() {
				...
				channel = bootstrap.connect(addressProvider.getHost(),
						addressProvider.getPort()).channel();
				System.out.println("bootstrap.connect");
	...
	private void reconnect(Channel channel) {
		...
		/*
		 * synchronized(channel){ if( channelList.indexOf(channel) == -1){
		 * System.out.println("channelList.indexOf(channel) == -1"); return; } }
		 */

		/*
		 * Channel newChannel = bootstrap.connect(addressProvider.getHost(),
		 * addressProvider.getPort()).channel();
		 * System.out.println("client 重新连接到: " + addressProvider.getHost() +
		 * ", " + addressProvider.getPort());
		...
		// System.out.println("client 尝试重新连接 : " + addressProvider.getHost() +
		// ", " + addressProvider.getPort());
